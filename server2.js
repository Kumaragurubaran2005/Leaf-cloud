...
// IMPROVED DATASET SPLITTING FUNCTION
function splitDataset(buffer, numParts, fileType = 'auto') {
  if (!buffer || buffer.length === 0) {
    console.log(`📊 No dataset provided, returning ${numParts} empty chunks`);
    return Array(numParts).fill(null);
  }
  
  console.log(`📊 Splitting dataset of ${buffer.length} bytes into ${numParts} parts`);
  
  // Detect file type if auto
  if (fileType === 'auto') {
    const firstBytes = buffer.slice(0, 100).toString();
    if (firstBytes.includes(',') && firstBytes.split('\n')[0].split(',').length > 1) {
      fileType = 'csv';
    } else if (firstBytes.includes('\t') && firstBytes.split('\n')[0].split('\t').length > 1) {
      fileType = 'tsv';
    } else if (firstBytes.trim().startsWith('{') || firstBytes.trim().startsWith('[')) {
      fileType = 'json';
    } else {
      fileType = 'text';
    }
  ...
  try {
    if (fileType === 'csv' || fileType === 'tsv') {
      return splitTextFileByLines(buffer, numParts, fileType);
    } else if (fileType === 'json') {
      return splitJsonFile(buffer, numParts);
    } else {
      return splitBinaryFile(buffer, numParts);
    }
  } catch (error) {
    console.error("Error splitting dataset, falling back to binary split:", error.message);
    return splitBinaryFile(buffer, numParts);
  }
}


// Split text files (CSV, TSV) by lines while preserving headers
function splitTextFileByLines(buffer, numParts, fileType) {
  ...
  if (lines.length <= numParts) {
    console.log(`📊 Fewer lines than workers, some workers will get empty datasets`);
    const chunks = [];
    for (let i = 0; i < numParts; i++) {
      chunks.push(i < lines.length ? Buffer.from(lines[i] + '\n') : null);
    }
    return chunks;
  }
  
  // Check if first line is header
  const hasHeader = lines[0].split(delimiter).length > 1 && 
                   lines[0].split(delimiter).every(field => field.trim().length > 0);
  
  const header = hasHeader ? lines[0] : null;
  const dataLines = hasHeader ? lines.slice(1) : lines;
  
  const linesPerChunk = Math.ceil(dataLines.length / numParts);
  const chunks = [];
  ...
  for (let i = 0; i < numParts; i++) {
    ...
    // Add newline at end if original had it
    if (text.endsWith('\n')) {
      chunkContent += '\n';
    }
    
    chunks.push(Buffer.from(chunkContent));
    console.log(`📊 Chunk ${i}: ${chunkLines.length} lines, ${chunkContent.length} bytes`);
  }
  
  return chunks;
}

// Split JSON files (array of objects)
function splitJsonFile(buffer, numParts) {
  const text = buffer.toString();
  let data;
  ...
  try {
    ...
    console.error("Invalid JSON, falling back to text split:", error.message);
    return splitTextFileByLines(buffer, numParts, 'text');
  }
  
  if (!Array.isArray(data)) {
    console.log("JSON is not an array, treating as single object");
    // For single JSON object, only first worker gets it
    const chunks = [buffer];
  ...
  const itemsPerChunk = Math.ceil(data.length / numParts);
  const chunks = [];
  ...
  for (let i = 0; i < numParts; i++) {
    ...
    chunks.push(Buffer.from(JSON.stringify(chunkData, null, 2)));
    console.log(`📊 JSON Chunk ${i}: ${chunkData.length} items`);
  }
  
  return chunks;
}

// Split binary files evenly
function splitBinaryFile(buffer, numParts) {
  ...
  for (let i = 0; i < numParts; i++) {
    ...
    console.log(`📊 Binary Chunk ${i}: ${end - start} bytes (${start}-${end})`);
  }
...
function combineWorkerResults(workerResults, originalFileType = 'auto') {
  console.log(`🔄 Combining results from ${Object.keys(workerResults).length} workers`);
  
  const workerIds = Object.keys(workerResults);
  if (workerIds.length === 0) {
    return Buffer.from("No results available from workers");
  }
  
  // If only one worker, return their result directly
  if (workerIds.length === 1) {
    const singleResult = workerResults[workerIds[0]];
    console.log(`🔄 Single worker result: ${singleResult.length} bytes`);
    return singleResult;
  }
  
  try {
    // Try to detect result type from first worker's result
    const firstResult = workerResults[workerIds[0]].toString();
    
    if (originalFileType === 'csv' || (firstResult.includes(',') && firstResult.split('\n')[0].split(',').length > 1)) {
      return combineCsvResults(workerResults);
    } else if (originalFileType === 'json' || (firstResult.trim().startsWith('[') || firstResult.trim().startsWith('{'))) {
      return combineJsonResults(workerResults);
    } else {
      return combineTextResults(workerResults);
    }
  } catch (error) {
    console.error("Error combining results, falling back to text combination:", error.message);
    return combineTextResults(workerResults);
  }
}

// Combine CSV results
function combineCsvResults(workerResults) {
  const workerIds = Object.keys(workerResults);
  let combined = '';
  let headerAdded = false;
  
  for (const workerId of workerIds) {
    const resultText = workerResults[workerId].toString().trim();
    if (!resultText) continue;
    
    const lines = resultText.split('\n');
    
    if (!headerAdded && lines.length > 0) {
      // Add header from first worker
      combined += lines[0] + '\n';
      headerAdded = true;
    }
    
    // Add data lines (skip header for all workers)
    const dataLines = lines.length > 1 ? lines.slice(1) : [];
    if (dataLines.length > 0) {
      combined += dataLines.join('\n') + '\n';
    }
    
    console.log(`📊 Worker ${workerId}: ${dataLines.length} data lines`);
  }
  
  console.log(`🔄 Combined CSV: ${combined.split('\n').length} total lines`);
  return Buffer.from(combined.trim());
}

// Combine JSON results
function combineJsonResults(workerResults) {
  const workerIds = Object.keys(workerResults);
  const combinedArray = [];
  ...
  for (const workerId of workerIds) {
    ...
    try {
      const workerData = JSON.parse(resultText);
      if (Array.isArray(workerData)) {
        combinedArray.push(...workerData);
        console.log(`📊 Worker ${workerId}: ${workerData.length} JSON items`);
      } else {
        combinedArray.push(workerData);
        console.log(`📊 Worker ${workerId}: 1 JSON object`);
      }
    } catch (error) {
      console.error(`Error parsing JSON from worker ${workerId}:`, error.message);
      // If JSON parsing fails, treat as text
      combinedArray.push({ worker: workerId, data: resultText });
    }
  }
  
  console.log(`🔄 Combined JSON: ${combinedArray.length} total items`);
  return Buffer.from(JSON.stringify(combinedArray, null, 2));
}

// Combine text results
function combineTextResults(workerResults) {
  const workerIds = Object.keys(workerResults);
  let combined = '';
  let workerCount = 0;
  
  combined += `COMBINED RESULTS FROM ${workerIds.length} WORKERS\n`;
  combined += '=' .repeat(50) + '\n\n';
  ...
  for (const workerId of workerIds) {
    ...
    combined += `WORKER ${workerId} RESULTS:\n`;
    combined += '-'.repeat(30) + '\n';
    combined += resultText + '\n\n';
    workerCount++;
  }
  
  combined += `\nTOTAL: Processed by ${workerCount} workers\n`;
  console.log(`🔄 Combined text: ${combined.length} bytes from ${workerCount} workers`);
...
// Submit package - IMPROVED DATASET SPLITTING
app.post(
  "/sendingpackage",
  authenticateJWT,
  ...
  async (req, res) => {
    try {
      ...
      // IMPROVED: Split dataset properly with file type detection
      const datasetChunks = files.dataset
        ? splitDataset(files.dataset[0].buffer, numWorkers)
        : Array(numWorkers).fill(null);

      // Log chunk information
      console.log(`📊 Dataset splitting summary for ${customerId}:`);
      datasetChunks.forEach((chunk, index) => {
        if (chunk) {
          console.log(`  Chunk ${index}: ${chunk.length} bytes`);
        } else {
          console.log(`  Chunk ${index}: EMPTY`);
        }
      });
      ...
      customers[customerId] = {
        ...
        isCancelled: false,
        createdAt: new Date(),
        // Store dataset info for result combination
        datasetInfo: {
          hasDataset: !!files.dataset,
          originalSize: files.dataset ? files.dataset[0].buffer.length : 0,
          fileType: files.dataset ? 'auto' : 'none'
        }
      };
      ...
      for (let i = 0; i < numWorkers; i++) {
        taskQueue.push({ 
          customerId, 
          taskId,
          chunkIndex: i // Track which chunk this task corresponds to
        });
      }
      ...
      console.log(`📁 Dataset split into ${datasetChunks.filter(chunk => chunk).length} non-empty chunks`);
      ...
      res.json({ 
        ...
        originalFilenames,
        datasetInfo: {
          chunks: datasetChunks.map(chunk => chunk ? chunk.length : 0),
          totalSize: files.dataset ? files.dataset[0].buffer.length : 0
        }
      });
    } catch (error) {
...
// Get a task - IMPROVED CHUNK ASSIGNMENT
app.post("/gettask", async (req, res) => {
  ...
  // IMPROVED: Use the chunk index from the task to ensure correct assignment
  const chunkIndex = task.chunkIndex;
  const datasetChunk = customerTask.files.datasetChunks[chunkIndex];

  console.log(`🔧 Assigning worker ${workerId} to chunk ${chunkIndex} (${datasetChunk ? datasetChunk.length + ' bytes' : 'EMPTY'})`);

  // Assign worker to task with chunk index
  customerTask.workers[chunkIndex] = workerId; // Store by chunk index to maintain order
  customerTask.workerHeartbeats[workerId] = Date.now();

  // Store which chunk this worker is processing
  customerTask.workerChunks = customerTask.workerChunks || {};
  customerTask.workerChunks[workerId] = chunkIndex;
  ...
  addProgressUpdate(
    task.customerId, 
    `🔧 Worker ${workerId} assigned to process dataset chunk ${chunkIndex + 1}/${customerTask.numWorkers}. Progress: ${progress.submitted}/${progress.total} workers completed.`,
    progress
  );

  console.log(`🔧 Worker ${workerId} assigned to customer ${task.customerId} (chunk ${chunkIndex + 1}/${customerTask.numWorkers})`);
  ...
  res.json({
    ...
    originalFilenames: customerTask.originalFilenames,
    assignment: {
      chunkIndex,
      totalWorkers: customerTask.numWorkers,
      hasDataset: !!datasetChunk,
      datasetSize: datasetChunk ? datasetChunk.length : 0
    }
  });
});
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Check if worker is authorized for this task
      ...
      let updateMessage = `Worker ${workerId} completed processing their dataset chunk. Progress: ${submittedResults}/${totalWorkers} workers.`;
      if (Object.keys(outputFiles).length > 0) {
      ...
      if (areAllResultsAvailable(customerId) && !customerTask.isCompleted) {
        ...
        console.log(`🔄 Combining results from ${submittedResults} workers for customer ${customerId}`);
        customerTask.combinedResult = combineWorkerResults(customerTask.results, customerTask.datasetInfo.fileType);
        console.log(`✅ Combined result created: ${customerTask.combinedResult.length} bytes`);
        
        // Send completion notification
        addCompletionNotification(customerId);
        customerTask.completionNotified = true;
        
        console.log(`🎉 Task ${customerId} completed by all workers - results combined and client notified`);
      }
...
// Get results (with combined results from all workers)
app.get("/getresults/:customerId", authenticateJWT, (req, res) => {
  const { customerId } = req.params;
  const customerTask = customers[customerId];
  ...
  try {
    ...
    // Add COMBINED result as the main result file
    if (customerTask.combinedResult) {
      archive.append(customerTask.combinedResult, { name: `combined_final_result.txt` });
      console.log(`📦 Added combined result: ${customerTask.combinedResult.length} bytes`);
    }

    // Add individual worker results for reference
    Object.keys(customerTask.results).forEach(workerId => {
      const resultBuffer = customerTask.results[workerId];
      const usageBuffer = customerTask.usage[workerId];
      const chunkIndex = customerTask.workerChunks ? customerTask.workerChunks[workerId] : 'unknown';

      if (resultBuffer) {
        archive.append(resultBuffer, { name: `individual_results/worker_${workerId}_chunk_${chunkIndex}_result.txt` });
      }
      if (usageBuffer) {
        archive.append(usageBuffer, { name: `usage/worker_${workerId}_chunk_${chunkIndex}_usage.txt` });
      }
      ...
      if (customerTask.outputFiles && customerTask.outputFiles[workerId]) {
        const workerOutputFiles = customerTask.outputFiles[workerId];
        Object.keys(workerOutputFiles).forEach(filename => {
          archive.append(workerOutputFiles[filename], { name: `output/${workerId}_chunk_${chunkIndex}/${filename}` });
        });
      }
    });

    // Add a comprehensive summary file
    const workerSummaries = Object.keys(customerTask.results).map(workerId => {
      const chunkIndex = customerTask.workerChunks ? customerTask.workerChunks[workerId] : 'unknown';
      const resultSize = customerTask.results[workerId] ? customerTask.results[workerId].length : 0;
      const outputFiles = customerTask.outputFiles && customerTask.outputFiles[workerId] 
        ? Object.keys(customerTask.outputFiles[workerId]) 
        : [];
      
      return `✓ Worker ${workerId} (Chunk ${chunkIndex}) - Result: ${resultSize} bytes, Output files: ${outputFiles.length > 0 ? outputFiles.join(', ') : 'None'}`;
    }).join('\n');

    const summary = `DISTRIBUTED PROCESSING COMPLETION SUMMARY
============================================
...
Total Workers: ${customerTask.numWorkers}
Workers Completed: ${Object.keys(customerTask.results).length}
Dataset Processed: ${customerTask.datasetInfo.hasDataset ? 'Yes' : 'No'}
Dataset Size: ${customerTask.datasetInfo.originalSize} bytes
Task Created: ${customerTask.createdAt.toISOString()}
Task Completed: ${customerTask.completedAt?.toISOString() || new Date().toISOString()}
...
COMBINED RESULT:
- combined_final_result.txt: Final combined result from all workers

FILES INCLUDED:
- combined_final_result.txt     - Final combined result
- individual_results/           - Individual worker results
- usage/                        - Individual worker usage reports  
- output/                       - Output files generated by workers
- This summary file

PROCESSING METHOD:
- Dataset was split into ${customerTask.numWorkers} chunks
- Each worker processed their assigned chunk independently
- Results were combined automatically to produce the final output

NOTE: The combined_final_result.txt contains the complete processed result.
Individual worker results are provided for reference and debugging.
`;
    archive.append(summary, { name: `processing_summary.txt` });
    ...
    const metadata = {
      ...
      customerName: customerTask.cusname,
      numWorkers: customerTask.numWorkers,
      workers: customerTask.workers.filter(w => w), // Remove empty slots
      workerAssignments: customerTask.workerChunks,
      datasetInfo: customerTask.datasetInfo,
      combinedResultSize: customerTask.combinedResult ? customerTask.combinedResult.length : 0,
      completedAt: customerTask.completedAt?.toISOString() || new Date().toISOString(),
      fileStructure: {
        "combined_final_result.txt": "Final combined result from all workers",
        "individual_results/": "Individual worker results for each dataset chunk",
        "usage/": "Individual worker resource usage reports",
        "output/": "Output files generated by workers"
      }
    };
    archive.append(JSON.stringify(metadata, null, 2), { name: `metadata.json` });
    ...
    archive.on('end', () => {
      console.log(`📥 Sent enhanced results ZIP for customer ${customerId} with combined result`);
    });

    archive.finalize();
    
    console.log(`📥 Sending results ZIP for customer ${customerId} with ${Object.keys(customerTask.results).length} worker results`);
    console.log(`📦 Combined result size: ${customerTask.combinedResult ? customerTask.combinedResult.length : 0} bytes`);
  ...
  for (const task of [...taskProgressQueue]) {
    ...
    for (const workerId of [...customerTask.workers]) {
      ...
      if (now - lastBeat > HEARTBEAT_TIMEOUT) {
        ...
        // Get the chunk index this worker was processing
        const chunkIndex = customerTask.workerChunks ? customerTask.workerChunks[workerId] : 0;
        
        // Remove worker from task
        customerTask.workers[chunkIndex] = null; // Set to null to maintain array structure
        delete customerTask.workerHeartbeats[workerId];
        if (customerTask.workerChunks) {
          delete customerTask.workerChunks[workerId];
        }
        ...
        // Add task back to queue for reassignment with the same chunk index
        taskQueue.push({ 
          customerId: task.customerId, 
          taskId: task.taskId,
          chunkIndex: chunkIndex
        ...
        addProgressUpdate(
          task.customerId, 
          `⚠️ Worker ${workerId} timed out while processing chunk ${chunkIndex + 1}. Reassigning task...`
        );

        console.log(`🔄 Task for chunk ${chunkIndex} reassigned for customer ${task.customerId} after worker ${workerId} timeout`);
      }
    }
  }
}, 5000);
...
app.listen(PORT, () => {
  ...
  console.log(`🎯 DISTRIBUTED PROCESSING: ENABLED`);
  console.log(`📦 Dataset splitting: ACTIVE (CSV, JSON, Text, Binary)`);
  console.log(`🔄 Result combination: ACTIVE`);
  console.log(`📈 Worker usage analytics: ENABLED`);
  console.log(`🔒 Worker validation: ACTIVE`);
